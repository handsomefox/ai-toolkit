...
Prefer the built-in tools over shell equivalents: Read over `cat`, Glob over
`fd` or `find`, Grep over `rg`, Edit and Write for deliberate file edits. They
return structured results, respect ignore files, and keep edits reviewable.
...
- **Structural search**: `ast-grep --lang <language> -p '<pattern>'` matches
  code shape rather than text. Use it for queries Grep cannot express — every
...
- **Symbol queries** — definition, references, implementations, call
  hierarchy — go through the LSP tools, not `ast-grep` and not Grep. LSP
  resolves through aliases, embedding, and re-exports; text and AST search do
  not.
- **JSON**: `jq`. **YAML, XML, TOML**: `yq`.
- `fd` when you need file metadata Glob does not expose: modification time,
